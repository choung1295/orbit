...
/**
 * 채팅 메인 레이아웃 컴포넌트
 * - conversationId는 URL query param ?chat=<id> 에서 읽음
 * - 각 브라우저 창은 독립된 URL을 가져 대화 상태가 분리됨
 * - 새로고침/직접 URL 접근 시 해당 conversation 복원
 */
export default function OrbitChat() {
    ...
    // URL에서 conversationId 읽기 (단일 진실 공급원)
    ...
    // 새 대화 → URL에서 chat 파라미터 제거
    const handleNewChat = () => {
        router.push('/orbit')
    }

    // 대화 선택 → URL 이동 (뒤로가기 히스토리 생성)
    const handleSelectChat = (id: string) => {
        router.push('/orbit?chat=' + id)
    }

    // 새 대화 생성 완료 → URL 교체 (뒤로가기 히스토리 미생성 - 빈 채팅으로 되돌아가지 않도록)
    const handleConversationCreated = (id: string) => {
        router.replace('/orbit?chat=' + id)
    }
